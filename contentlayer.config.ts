...
// Tag normalization map (duplicated from lib/taxonomy.ts to avoid contentlayer import issues)
 ...
 * Uses normalized tag names as keys for consistency.
 */
  ...
  allBlogs.forEach((file) => {
    if (file.tags && (!isProduction || file.draft !== true)) {
      file.tags.forEach((tag) => {
        // Normalize tag name for consistent display
        const normalizedTag = normalizeTag(tag)
        if (normalizedTag in tagCount) {
...
export const Authors = defineDocumentType(() => ({
  ...
  fields: {
    ...
    education1Desc: { type: 'string' },
    education2: { type: 'string' },
    education2Desc: { type: 'string' },
  },
  computedFields,
}))
